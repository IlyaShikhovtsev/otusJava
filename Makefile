...
build:
	./mvnw verify
	chmod +x ./target/otus-java-0.0.1-SNAPSHOT.jar

run:
	./mvnw spring-boot:start -Dspring.profiles.active=local
#	./target/otus-java-0.0.1-SNAPSHOT.jar
#	java -jar --enable-preview ./target/otus-java-0.0.1-SNAPSHOT.jar
...
git-fork-init: init
	git remote add upstream git://github.com/petrelevich/otus_java_2019_09.git
	git fetch upstream
